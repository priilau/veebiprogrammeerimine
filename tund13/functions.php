<?php
  ...
  function showmyprofile(){
	...
	if(!empty($profile->picture)){
	  $stmt = $mysqli->prepare("SELECT filename FROM vp_user_profilepic WHERE id=?");
	  echo $mysqli->error;
  ...
  //kasutajaprofiili salvestamine
	...
	function addPhotoData($filename, $alttext, $privacy){
		...
		if($stmt->execute()){
			$notice= "Andmebaasiga on kõik korras!";
		} else {
			$notice="Andmebaasiga läks midagi viltu!" .$stmt->error;
		}
		$stmt->close();
		$mysqli->close();
		return $notice;
	...
	function addUserPhotoData($fileName){
		$addedId = null;
		$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
		$stmt = $mysqli->prepare("INSERT INTO vp_user_profilepic (userid, filename) VALUES (?, ?)");
		echo $mysqli->error;
	...
	function lastPicture($privacy){
		...
		if($stmt->fetch()){
			...
			$html = "<p>Avalikke pilte ei leidu!</p>\n";
		}
		$stmt->close();
		$mysqli->close();
		return $html;
	...
	function allPublicPicturesThumbs($privacy){
		$privacy = 2;
		$html = "";
		$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
	...
	function allPublicPicturesThumbsPage($page, $limit){
		$privacy = 2;
		$skip = ($page - 1) * $limit;
		$html = "";
		$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
