<?php 
	...
	public function createThumbnail($directory, $size){
		...
		$notice = $this->savePicFile($myThumbnail, $target_file);
		return $notice;
	}
	public function savePhoto($target_file){
		$notice = $this->savePicFile($this->myImage, $target_file);
		return $notice;
	}
	public function savePicFile($image, $target_file){
		$notice = null;
		...
		if($this->imageFileType == "png"){
			if(imagepng($image, $target_file, 6)){
				$notice = 1;
			} else {
				$notice = 0;
			}	
